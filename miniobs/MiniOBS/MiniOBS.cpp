...
void MiniOBS::bind(HWND hwnd)
{
	...
	// Start output
	if(dict[L"output_shared_memory"]!=L"0")
	{
		Log::i(TAG, L"Enable output to shared memory");
		obs_data_t *settings = obs_data_create();
		obs_output_t *output = obs_output_create("shared_memory_output", "shared_memory_output", settings, NULL);
